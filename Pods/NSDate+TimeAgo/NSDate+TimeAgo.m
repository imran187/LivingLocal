...
#define NSDateTimeAgoLocalizedStrings(key)                                     \
  NSLocalizedStringFromTableInBundle(                                          \
      ...
      [NSBundle                                                                \
          bundleWithPath:                                                      \
              [[[NSBundle bundleForClass:[DummyClass class]] resourcePath]     \
                  stringByAppendingPathComponent:@"NSDateTimeAgo.bundle"]],    \
      nil)
#endif
...
- (NSString *)timeAgo {
  NSDate *now = [[NSDate date] toGlobalTime];
  double deltaSeconds = fabs([self timeIntervalSinceDate:now]);
  double deltaMinutes = deltaSeconds / 60.0f;
  ...
  if (deltaSeconds < 5) {
    ...
    return [self stringFromFormat:@"Just now" withValue:deltaSeconds];
  } else if (deltaSeconds < 120) {
    return NSDateTimeAgoLocalizedStrings(@"1 min ago");
  } else if (deltaMinutes < 60) {
    return [self stringFromFormat:@"%%d %@min ago" withValue:deltaMinutes];
  } else if (deltaMinutes < 120) {
    return NSDateTimeAgoLocalizedStrings(@"1 hr ago");
  } else if (deltaMinutes < (24 * 60)) {
    minutes = (int)floor(deltaMinutes / 60);
    return [self stringFromFormat:@"%%d %@hrs ago" withValue:minutes];
  } else if (deltaMinutes < (24 * 60 * 2)) {
    df.dateFormat = @"hh:mm a";
    return NSDateTimeAgoLocalizedStrings(
        [@"Yesterday at " stringByAppendingString:[df stringFromDate:self]]);
  } else if (deltaMinutes < (24 * 60 * 7)) {
    df.dateFormat = @"MMMM d 'at' hh:mm a";
...
- (NSString *)dateTimeAgo {
  ...
  } else if (components.minute >= 1) // up to 59 minutes ago
  {
    ...
    return [self stringFromFormat:@"%%d %@minutes ago"
                        withValue:components.minute];
  ...
  return
      [self stringFromFormat:@"%%d %@seconds ago" withValue:components.second];
}
...
- (NSString *)dateTimeUntilNow {
  ...
  if (components.hour >= 6) // if more than 6 hours ago, change precision
  {
    ...
    if (diffDays == 0) // today!
    {
      NSDateComponents *startHourComponent =
          [calendar components:NSCalendarUnitHour fromDate:self];
      NSDateComponents *endHourComponent =
          [calendar components:NSCalendarUnitHour fromDate:self];
      if (startHourComponent.hour < 12 && endHourComponent.hour > 12) {
        return NSDateTimeAgoLocalizedStrings(@"This morning");
                 ...
                 startHourComponent.hour < 18 && endHourComponent.hour >= 18) {
...
- (NSString *)timeAgoWithLimit:(NSTimeInterval)limit {
  return [self timeAgoWithLimit:limit
                     dateFormat:NSDateFormatterFullStyle
                  andTimeFormat:NSDateFormatterFullStyle];
}

- (NSString *)timeAgoWithLimit:(NSTimeInterval)limit
                    dateFormat:(NSDateFormatterStyle)dFormatter
                 andTimeFormat:(NSDateFormatterStyle)tFormatter {
