...
@interface CustomeImagePicker () <UIImagePickerControllerDelegate>
@property(nonatomic, strong) NSArray *assets;
// @property(nonatomic,strong) UIImage *selectedImage;
@end

@implementation CustomeImagePicker
@synthesize whereamI, skipButton, nextButton, hideNextButton, hideSkipButton,
    highLightThese, maxPhotos, distanceFromButton, nearByCollectionView,
    currentUserLocation, showOnlyPhotosWithGPS;
...
- (void)viewDidAppear:(BOOL)animated {
  ...
  //  if(showOnlyPhotosWithGPS==NO && [self checkForCamera] == YES)
    if ([self checkForCamera]) {
        [self performSelector:@selector(initializeCamera)
                   withObject:nil
                   afterDelay:0.1];
    }
}
...
- (void)viewDidLoad {
  ...
  [self.collectionView registerClass:[PhotoPickerCell class]
          forCellWithReuseIdentifier:@"PhotoPickerCell"];
  [self.collectionView registerClass:[CameraCell class]
  ...
  //  dispatch_time_t popTime1 = dispatch_time(DISPATCH_TIME_NOW, 1 *
  //  NSEC_PER_SEC);
...
- (void)loadAssets {
  ...
  [assetsLibrary enumerateGroupsWithTypes:ALAssetsGroupSavedPhotos
      ...
      failureBlock:^(NSError *error) {
        ...
        if ([ALAssetsLibrary authorizationStatus] !=
          ...
          [self displayErrorOnMainQueue:@"Photo Access Disabled"
                                message:@"Please allow Photo Access in System "
                                        @"Settings"];
        }
      }];
  ...
  dispatch_after(popTime1, dispatch_get_main_queue(), ^(void) {
    if (showOnlyPhotosWithGPS == YES) {
      OLGhostAlertView *ghastly = [[OLGhostAlertView alloc]
          initWithTitle:@"Photos with Location"
                message:@"Only Photos with Location Information are shown here."
                timeout:2.0
            dismissible:YES];
      [ghastly show];
    }
...
- (void)displayErrorOnMainQueue:(NSString *)heading
                              ...
                              usingBlock:^(ALAsset *result, NSUInteger index,
                                ...
                                if (result) {
                                  NSDictionary *metadata =
                                      [[result defaultRepresentation] metadata];
                                  ...
                                  if (GPSDictionary) {
                                    ...
                                    CLLocation *locFromPhoto = [
                                        [CLLocation alloc]
                                        initWithLatitude:
                                            [[GPSDictionary
                                                objectForKey:@"Latitude"]
                                                doubleValue]
                                               longitude:[[GPSDictionary
                                                             objectForKey:
                                                                 @"Longitude"]
                                                             doubleValue]];
                                    ...
                                    CLLocation *userLocation =
                                        [[CLLocation alloc]
                                            initWithLatitude:loc.latitude
                                                   longitude:loc.longitude];

                                    CLLocationDistance distance = [locFromPhoto
                                        distanceFromLocation:userLocation];
                                    NSLog(@"%f", distance);
                                    if (distance < 1000) {
                                      NSLog(@"Nearby");
                                    }
...
- (void)addLiveCamera {
  ...
  UICollectionViewCell *firstCell =
      [self.collectionView cellForItemAtIndexPath:ip];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
  ...
  if (indexPath.row == 0 && indexPath.section == 0 &&
      showOnlyPhotosWithGPS == NO) {
    CameraCell *cell = (CameraCell *)[collectionView
        dequeueReusableCellWithReuseIdentifier:cameraCellIdentifier
                                  forIndexPath:indexPath];

    return cell;
  } // Device with Camera
  else {
    PhotoPickerCell *cell = (PhotoPickerCell *)[collectionView
...
- (void)collectionView:(UICollectionView *)collectionView
  ...
  if (indexPath.row == 0 && indexPath.section == 0) {
    if ([self checkForCamera] == YES) {
      ...
      [self
          displayErrorOnMainQueue:@"Camera Access Disabled"
                          message:
    ...
    if (![highLightThese containsObject:assetURL] &&
        ([highLightThese count] < maxPhotos)) {
...
didFinishPickingMediaWithInfo:(NSDictionary *)info {
  ...
  [lib addAssetsGroupAlbumWithName:@"Living Local"
      resultBlock:^(ALAssetsGroup *group) {
        if (group == nil) {
          // enumerate albums
          [[ALAssetsLibrary new] enumerateGroupsWithTypes:ALAssetsGroupAlbum
              usingBlock:^(ALAssetsGroup *g, BOOL *stop) {
                // if the album is equal to our album
                if ([[g valueForProperty:ALAssetsGroupPropertyName]
                        isEqualToString:@"Living Local"]) {

                  // save image
                  [[ALAssetsLibrary new]
                      writeImageDataToSavedPhotosAlbum:UIImagePNGRepresentation(
                                                           pickedImage)
                                              metadata:nil
...
//  transform
//  // calculated above.
//  CGContextRef ctx = CGBitmapContextCreate(NULL, image.size.width,
//  image.size.height,
...
//      CGRectMake(0,0,image.size.height,image.size.width), image.CGImage);
//      break;
//
//    default:
//      CGContextDrawImage(ctx,
//      CGRectMake(0,0,image.size.width,image.size.height), image.CGImage);
//      break;
//  }
//
...
- (IBAction)cancelPressed:(id)sender {
  ...
  [self dismissViewControllerAnimated:YES
                           completion:^{
                             if ([self.delegate
                                     respondsToSelector:
                                         @selector(imageSelectionCancelled)]) {
...
- (void)initializeCamera {
  ...
  CameraCell *firstCell = (CameraCell *)[self.collectionView
                   ...
                   completionBlock:^(NSURL *assetURL, NSError *error) {
                     [self
                         dismissViewControllerAnimated:NO
                                            completion:^{
                                              NSMutableArray *allImagesIPicked =
                                                  [[NSMutableArray alloc] init];
                                              [allImagesIPicked
                                                  addObject:
                                                      assetURL.absoluteString];
                                              // Now insert the Camera Image at
                                              // 0 and 2 more
                                              if ([highLightThese count] >= 1) {
                                                int count = 1;
                                                for (NSString *ip in
...
- (void)didFinishPickingImage:(UIImage *)image
  ...
  dispatch_async(
      dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{ // 1
        ...
        NSDictionary *extraParams = [NSDictionary
            dictionaryWithObjects:[NSArray
                                      arrayWithObjects:image, metadata, nil]
                          forKeys:[NSArray arrayWithObjects:@"data",
                                                            @"metadata", nil]];

        [NSThread detachNewThreadSelector:@selector(doSomeThingWithImage:)
                                 toTarget:self
