...
{
  ParallaxHeaderView *headerView = [[ParallaxHeaderView alloc]
      initWithFrame:CGRectMake(0, 0, headerSize.width, headerSize.height)];
...
+ (id)parallaxHeaderViewWithSubView:(UIView *)subView {
  ParallaxHeaderView *headerView = [[ParallaxHeaderView alloc]
      initWithFrame:CGRectMake(0, 0, subView.frame.size.width,
...
- (void)layoutHeaderViewForScrollViewOffset:(CGPoint)offset {
  ...
  if (offset.y > 0) {
    ...
    self.bluredImageView.alpha =
        1 / kDefaultHeaderFrame.size.height * offset.y * 2;
...
- (void)refreshBlurViewForNewImage {
  ...
  screenShot =
      [screenShot applyBlurWithRadius:5
                            tintColor:[UIColor colorWithWhite:1 alpha:0]
...
- (void)initialSetupForDefaultHeader {
  ...
  UIImageView *imageView =
      [[UIImageView alloc] initWithFrame:scrollView.bounds];
